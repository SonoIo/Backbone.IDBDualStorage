;(function (root, factory) {
	...
	Backbone.Collection.prototype.syncDirty = function syncDirty(done) {
		...
		getDirtyModelIds(storeName, function (err, dirtyModelIds) {
			...
			eachSeries(arrayOfDirtyModels, save, function (err) {
				done(errors.length > 0 ? errors : null, response);
			});
		});

		function save(aModel, next) {
			aModel.save(null, {
				...
				// if the server respond with an invalid model success and error
				// callback wont be executed. With validate false no validation
				// will be performed and success will be triggered as expected
				validate: false
			});
		}

		function getDirtyModelIds(store, callback) {
	...
	Backbone.Collection.prototype.syncDestroyed = function syncDestroyed(done) {
		...
		function getDestroyedModelIds(store, callback) {
			DirtyStore.getInstance(function (err, store) {
				if (err) return callback(err);
			...
			switch (method) {
					...
					options.success = function (resp) {
						if (options.dirty) {
							store.addDirty(model, function (err) {
								return responseHandler(err, resp);
							});
						}
						else {
					...
					options.success = function (resp) {
						if (options.dirty) {
							if (model.hasTempId()) {
								...
								store.removeDirty(model, function (err, result) {
									if (err) return responseHandler(err, result);
				...
				case 'reset':
					...
					if (model instanceof Backbone.Collection) {
						...
						storeName = _.result(collection.model.prototype, 'idAttribute');
						store.reset(storeName, resetCallback);
					}
					else {
						storeName = _.result(model.prototype, 'idAttribute');
						store.reset(storeName, resetCallback);
					}
					break;
			}
